...
xRange <- abs(X[1,1]-X[1,2])
yRange <- abs(X[2,1]-X[2,2])

if (xRange > yRange) {
  xlims <- c(min(X[1,])-3, max(X[1,])+3)
  ylims <- c(min(X[2,])-((xRange-yRange)/2+3), max(X[2,])+((xRange-yRange)/2+3))
} else {
  xlims <- c(min(X[1,])-((yRange-xRange)/2+3), max(X[1,])+((yRange-xRange)/2+3))
  ylims <- c(min(X[2,])-3, max(X[2,])+3)
}
plot(0,0, cex=0, xlim=xlims, ylim=ylims)
if ((X[2,1] > X[2,2]) || (X[2,1] == X[2,2] && X[1,1] > X[1,2])) {
  points(X[1,1], X[2,1], col=cols[1], pch=16, cex=2)
  points(X[1,2], X[2,2], col=cols[2], pch=16, cex=2)
} else {
  points(X[1,1], X[2,1], col=cols[2], pch=16, cex=2)
  points(X[1,2], X[2,2], col=cols[1], pch=16, cex=2)
}
legend('topleft', legend=c('Class1','Class2','Boarder'), col=cols, pch=16)
...
repeat {
  ...
  print(y)
  signY <- sign(-X[2,1]+X[2,2])
  signX <- sign(-X[1,1]+X[1,2])
  if (y == 0) {
    class(x,3)
  } else if (y > 0) {
    if ((signY == 1) || (signY == 0 && signX == 1)) {
      class(x,1)
    } else {
      class(x,2)
    }
